...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = $conn->prepare("SELECT * FROM course_enrollments WHERE course_id = ? AND employee_id = ?");
    $check->bind_param("ii", $course_id, $employee_id);
    $check->execute();
    $result = $check->get_result();

    if ($result->num_rows > 0) {
        $message = "This employee is already enrolled in the course!";
    } else {
        // Enroll the employee
        $stmt = $conn->prepare("INSERT INTO course_enrollments (course_id, employee_id) VALUES (?, ?)");
...
// Fetch users excluding superadmin, admin, and officer
$employees = $conn->query("
    SELECT id, first_name, last_name, usertype 
    FROM users 
    WHERE usertype NOT IN ('superadmin', 'admin', 'officer')
");
?>
<!doctype html>
<html lang="en">
                ...
                <div class="container-fluid dashboard-content ">
                    <div class="row">
                        ...
                        <div class="row">
                            ...
                            <head>
    <title>Add Employee to Course</title>
    <style>
        body {
...
<div class="card-container">
    <h2>Add Employee/Staff to Course</h2>

    <?php if ($message): ?>
        <p style="color: <?= strpos($message, 'already') !== false ? 'red' : 'green' ?>; font-weight: bold;"><?= $message ?></p>
    <?php endif; ?>
        ...
        <select name="employee_id" required>
            ...
            <?php while ($emp = $employees->fetch_assoc()): ?>
                <option value="<?= $emp['id'] ?>">
                    ...
                    (<?= ucfirst($emp['usertype']) ?>)
                </option>
            <?php endwhile; ?>
        </select>
        <button type="submit" class="action-btn">Add to Course</button>
    </form>

    <a href="courses.php" class="action-btn" style="background-color: #6366f1; margin-top: 15px; display: inline-block;">← Back to Courses</a>
</div>
