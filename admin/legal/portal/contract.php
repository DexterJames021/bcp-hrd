...
// Get employee's ID
$employee_id = $_SESSION['user_id']; // Assuming user_id in session corresponds to employee_id in contracts

// Fetch employee's contracts
$sql = "SELECT c.contract_type, c.start_date, c.end_date, c.status, c.reminder_date, c.admin_notes, c.contract_file
        FROM contracts c
        WHERE c.employee_id = ?
        ORDER BY c.start_date DESC";

$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $employee_id);  // Bind employee ID to query
$stmt->execute();
...
<body>    <div class="dashboard-main-wrapper">
    ...
    <h2 class="mb-4 text-center">Your Contracts</h2>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($row['contract_file']): ?>
                                    <a href="<?= htmlspecialchars($row['contract_file']) ?>" target="_blank" class="btn btn-primary btn-sm">Download Contract</a>
                                <?php else: ?>
    ...
    <?php else: ?>
        <p class="alert alert-warning text-center">No contracts found.</p>
    <?php endif; ?>
</div>
