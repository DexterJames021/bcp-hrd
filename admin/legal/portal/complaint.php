...
if (!isset($_SESSION['user_id']) || $_SESSION['usertype'] !== 'employee') {
...
$message = "";
$employees_stmt = $conn->prepare("SELECT id, first_name, last_name FROM users WHERE usertype IN ('employee', 'staff')");
...
while ($row = $employees_result->fetch_assoc()) {
    ...
    $employeeOptions[$fullName] = $row['id'];
}

// INSERT COMPLAINT IF FORM IS SUBMITTED
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO complaints (complaint_type, description, submitted_by, against_employee, submitted_at, status) VALUES (?, ?, ?, ?, ?, 'Pending')");
    $stmt->bind_param("ssiis", $complaint_type, $description, $employee_id, $against_employee, $submitted_at);
    
    if ($stmt->execute()) {
        $message = "You have successfully submitted your complaint.";
    } else {
        $message = "Error submitting complaint: " . $conn->error;
    }
}

// FETCH COMPLAINTS TO DISPLAY
$sql = "SELECT c.complaint_type, c.description, c.status, c.admin_response, c.submitted_at
        FROM complaints c
...
<body>    
    <div class="dashboard-main-wrapper">
                        ...
                        <ul class="navbar-nav flex-column">
                            ...
                            <li class="nav-item ">
                                ...
                                <div id="submenu-3" class="collapse submenu" style="">
                                    <ul class="nav flex-column">
                                    <li class="nav-item">
                                            <a class="nav-link" href="employeesmanual.php">Employee's Manual</a>
                                        </li>
                                        <li class="nav-item">
        ...
        <form method="POST" action="">
                        <div class="mb-3">
                            <label for="against_employee_display" class="form-label">Against (Employee/Staff)</label>
                            <input class="form-control" list="employeeList" id="against_employee_display" placeholder="" required>
                            <datalist id="employeeList">
                                <?php foreach ($employeeOptions as $name => $id): ?>
                                    <option data-id="<?= $id ?>" value="<?= htmlspecialchars($name) ?>"></option>
                                <?php endforeach; ?>
                            </datalist>
                            <!-- Hidden input to hold the selected user's ID -->
                            <input type="hidden" name="against_employee" id="against_employee">
                        </div>
            <div class="mb-3">
    ...
    <script>
    document.getElementById('against_employee_display').addEventListener('input', function () {
        const input = this.value;
        const datalist = document.getElementById('employeeList').options;
        let selectedId = '';
        ...
        document.getElementById('against_employee').value = selectedId;
    });
</script>
</body>
