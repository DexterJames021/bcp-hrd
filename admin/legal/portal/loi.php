...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $loi_type = $_POST['loi_type']; // loi_type is still needed
    $status = 'Pending'; // Default status
    $submitted_by = $_SESSION['user_id']; // Get the logged-in user ID from the session
    $submitted_at = date('Y-m-d H:i:s');

    if ($message) {
        // Prepare and execute the query to insert data into the table
        $stmt = $conn->prepare("INSERT INTO letter_of_intent (loi_type, message, status, submitted_by, submitted_at) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssi", $loi_type, $message, $status, $submitted_by, $submitted_at);
...
// Query to fetch the LOI records for the logged-in employee
$query = "SELECT * FROM letter_of_intent WHERE submitted_by = ? ORDER BY submitted_at DESC";
...
<body>    <div class="dashboard-main-wrapper">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white">
            <h4 class="mb-0">Submit a Letter of Intent</h4>
        </div>
        <div class="card-body">
            <form method="POST" enctype="multipart/form-data">
                <div class="mb-4">
                    <label for="loi_type" class="form-label">LOI Type</label>
                    <select class="form-select" name="loi_type" required>
                ...
                <div class="mb-4">
                    <label for="upload" class="form-label">Upload Document (Optional)</label>
                    <input type="file" class="form-control" name="upload">
                </div>

                <div class="d-flex justify-content-between">
                    <button type="submit" class="btn btn-primary">Submit LOI</button>
                </div>
            </form>
    ...
    <!-- Table to display submitted LOIs and Admin Responses -->
    <div class="mt-5">
        <h4>Your Submitted Letters of Intent</h4>
        <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                <?php 
                // Query to get the employee's submitted LOIs
                $sql = "SELECT loi_type, message, status, admin_response, submitted_at 
                ...
                if ($result->num_rows > 0): 
                    while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?= htmlspecialchars($row['loi_type']) ?></td>
                            <td><?= htmlspecialchars($row['message']) ?></td>
                            <td>
                                <?php 
                                    // Show the status (Pending, Approved, or Rejected)
                                    echo htmlspecialchars($row['status']);
                                ?>
                            </td>
                            <td>
                                <?php 
                                    // Show the admin's response (if any)
                    ...
                    <tr><td colspan="5">No Letters of Intent submitted yet.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
