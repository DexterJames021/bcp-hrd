...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['loi_id'])) {
    $loi_id = $_POST['loi_id'];
    $status = $_POST['status'];
    $response = $_POST['admin_response'];

    $update = $conn->prepare("UPDATE letter_of_intent SET status = ?, admin_response = ? WHERE id = ?");
    $update->bind_param("ssi", $status, $response, $loi_id);
    $update->execute();
}

// Fetch all submitted LOIs
$sql = "SELECT loi.id, loi.loi_type, loi.message, loi.status, loi.submitted_at, loi.admin_response, loi.file_path, 
               u.first_name AS submitted_by_first_name, u.last_name AS submitted_by_last_name
        FROM letter_of_intent loi
        LEFT JOIN users u ON loi.submitted_by = u.id
...
<body>
    ...
    <div class="bg-primary p-4 rounded shadow-sm mb-4">
        <h2 class="text-white">Manage Letters of Intent</h2>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover">
            <thead class="table-primary">
                <tr>
                    <th class="text-dark">LOI Type</th>
                    <th class="text-dark">Message</th>
                    <th class="text-dark">Uploaded File</th>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars($row['submitted_by_first_name'] . ' ' . $row['submitted_by_last_name']) ?></td>
                            ...
                            <td><?= date('F j, Y, g:i a', strtotime($row['submitted_at'])) ?></td> <!-- Correct format -->
                            <td>
                                <form method="POST" action="">
                                    <input type="hidden" name="loi_id" value="<?= $row['id'] ?>">
                                    <select name="status" class="form-select mb-2">
                                        <option value="Pending" <?= $row['status'] == 'Pending' ? 'selected' : '' ?>>Pending</option>
                                        <option value="Approved" <?= $row['status'] == 'Approved' ? 'selected' : '' ?>>Approved</option>
                                        <option value="Rejected" <?= $row['status'] == 'Rejected' ? 'selected' : '' ?>>Rejected</option>
                                    </select>
                                    <textarea class="form-control mb-2" name="admin_response" rows="3" placeholder="Admin response"><?= $row['admin_response'] ?></textarea>
