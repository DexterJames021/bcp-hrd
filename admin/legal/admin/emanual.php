...
if(isset($_SESSION['usertype']) && $_SESSION['usertype'] != 'employee'){
    // Admin or other users can view this page
}else{
    header("Location: ../admin/forms/index.php"); // Redirect to another page for non-admin users
}
?>
...
<body>
    ...
    <div class="card shadow-lg">
        <div class="card-header bg-primary text-white text-center">
            <h4 class="mb-0">Employee's Manual</h4>
        </div>
        <div class="card-body">
            <h5 class="card-title text-center mb-4">Upload Document</h5>
            <form action="upload_manual.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-4">
                    <label for="manual" class="font-weight-bold">Upload PDF:</label>
                    <input type="file" class="form-control-file" id="manual" name="manual" accept=".pdf" required>
                </div>

                <div class="text-center">
                    <button type="submit" class="btn btn-outline-primary px-4">Upload</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card shadow-lg mt-5">
        <div class="card-header bg-primary text-white text-center">
            <h4 class="mb-0">Uploaded Employee Manuals</h4>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                        // Fetch the list of uploaded manuals from the database
                        // Assuming the table is named `manuals`
                        $conn = new mysqli("localhost", "root", "", "bcp-hrd"); // Adjust database connection details
                        if ($conn->connect_error) {
                            die("Connection failed: " . $conn->connect_error);
                        }
                        $sql = "SELECT * FROM employees_manual"; // Query to fetch uploaded manuals
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr>
                                    ...
                                    <td>
                                        <a href='edit_manual.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Edit</a>
                                        <a href='delete_manual.php?id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this file?\")'>Delete</a>
                                    </td>
                                </tr>";
                            }
                        } else {
                            echo "<tr><td colspan='4'>No manuals uploaded yet.</td></tr>";
                        }
                        $conn->close();
                        ?>
                    </tbody>
...
<style>
    ...
    .card-header {
        background-color: #0066cc; /* Same dark blue as other submenus */
        color: white;
    }
