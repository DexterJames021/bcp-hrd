...
if (isset($_GET['course_id'])) {
    ...
    $enrollments = $conn->query("
        SELECT u.id, u.first_name, u.last_name, u.email, u.phone, u.usertype
        FROM course_enrollments ce
        INNER JOIN users u ON ce.employee_id = u.id
        WHERE ce.course_id = $course_id
    ");

} else {
    echo "Invalid access. Course ID is missing.";
    exit;
}
?>
<!doctype html>
...
<body>
    ...
    <style>
        ...
        .back-btn { background-color: #6366f1; color: white; border: none; padding: 10px 14px; border-radius: 10px; font-size: 16px; cursor: pointer; text-decoration: none; }
...
<div class="card-container">
    <h2>Enrolled Employees — <?= htmlspecialchars($course['course_title']) ?></h2>
    ...
    <table>
        ...
        <tr>
            ...
            <td><?= htmlspecialchars(ucfirst($row['usertype'])) ?></td>
            <td><?= htmlspecialchars($row['email']) ?></td>
            <td><?= htmlspecialchars($row['phone']) ?></td>
            <td>
            <a href="delete_enrolled_employee.php?course_id=<?= $course_id ?>&employee_id=<?= $row['id'] ?>" class="action-btn" style="background-color: #ef4444; " onclick="return confirm('Are you sure you want to remove this employee from the course?');">🗑️ Delete</a>
        </td>
        </tr>
        <?php endwhile; ?>
    </table>
