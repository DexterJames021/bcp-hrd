...
if (isset($_GET['JobRoleID'])) {
    ...
    $stmt->bind_param("i", $JobRoleID); // Binding the parameter to prevent SQL injection
    $stmt->execute();
    ...
    if ($row['total'] > 0) {
        ...
        $deleteJobSql = "DELETE FROM jobroles WHERE JobRoleID = ?";
        $deleteStmt = $conn->prepare($deleteJobSql);
        $deleteStmt->bind_param("i", $JobRoleID); // Binding the parameter

        if ($deleteStmt->execute()) {
            $_SESSION['message'] = "Job role deleted successfully.";
    ...
    // Redirect back to the recruitment page after handling
