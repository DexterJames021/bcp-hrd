...
session_start();
$id = $_GET['id'];
$sql = "SELECT * FROM training_courses WHERE id = $id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_sql = "UPDATE training_courses SET 
...
<head>
    ...
    <style>
        ...
        .action-container {
    display: flex;
    justify-content: space-between; /* Ensures space between the link and the button */
    align-items: center; /* Vertically aligns the items */
}

        h2 { font-size: 20px; font-weight: 600; margin-bottom: 20px; color: #1e293b; }
        input, textarea { width: 100%; padding: 10px; margin-bottom: 15px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 16px; }
        .action-btn { background-color: #22c55e; color: white; border: none; padding: 10px 14px; border-radius: 10px; font-size: 16px; cursor: pointer; text-decoration: none; }
    </style>
</head>
<body>
    <div class="card-container">
        ...
        <!-- Flex container for aligning the items -->
        ...
        <form method="POST">
            ...
            <input type="date" name="start_date" value="<?= $course['start_date'] ?>" required>
            
            <label>End Date</label>
            <input type="date" name="end_date" value="<?= $course['end_date'] ?>" required>
    
            <input type="time" name="schedule_time" value="<?= $course['schedule_time'] ?>">
            <input type="text" name="instructor" value="<?= htmlspecialchars($course['instructor']) ?>">
            <input type="text" name="designation" value="<?= htmlspecialchars($course['designation']) ?>">
            <input type="text" name="location" value="<?= htmlspecialchars($course['location']) ?>">
            <div class="action-container">
                <a href="courses.php" class="action-btn" style="background-color: #6366f1;">← Back to Courses</a>
                <button type="submit" class="action-btn">Update Course</button>
            </div>
    </div>
    </body>
