...
$courses = $conn->query("SELECT * FROM training_courses ORDER BY start_date ASC");
?>
<!doctype html>
<html lang="en">
...
<html>
<head>
<title>Admin | Training Courses</title>
    <style>
        ...
        th { background-color: #f8fafc; color: #334155; font-size: 14px;  text-align: center;}
        ...
        .btn-employee { background-color: #22c55e; }
        .btn-view { background-color: #0ea5e9; }
        .btn:hover { opacity: 0.9; }
        th {
            width: 15%; /* Default width for other columns */
        }
        th:nth-child(1) { width: 10%; } /* Title */
        th:nth-child(2) { width: 10%; } /*Start Date */
        th:nth-child(3) { width: 10%; } /* End Date */
        th:nth-child(4) { width: 10%; } /* Time */
        th:nth-child(5) { width: 10%; } /* Instructor */
        th:nth-child(6) { width: 10%; } /* Designation */
        th:nth-child(7) { width: 10%; }  /* Minimized Location */
        th:nth-child(8) { width: 35%; } /* Enlarged Actions */
    </style>
...
<div class="card-container">
    ...
    <a href="add_course.php" class="add-btn">✚ Add New Training Course</a>
        ...
        <table>
            ...
            <?php while ($course = $courses->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><?= date('F j, Y', strtotime($course['end_date'])) ?></td>
                    <td><?= date('g:i A', strtotime($course['schedule_time'])) ?></td>
                    <td><?= htmlspecialchars($course['instructor']) ?></td>
                    <td><?= htmlspecialchars($course['designation']) ?></td>
                    <td><?= htmlspecialchars($course['location']) ?></td>
                    <td>
                        <a href="edit_course.php?id=<?= $course['id'] ?>" class="btn btn-edit">📝Edit</a>
                     
                        <a href="add_employee_course.php?course_id=<?= $course['id'] ?>" class="btn btn-employee">✚Employee</a>
                        <a href="enrolled_employees.php?course_id=<?= $course['id'] ?>" class="btn btn-view">📄View</a>
                        <a href="delete_course.php?id=<?= $course['id'] ?>" class="btn btn-delete" onclick="return confirm('Are you sure?')">🗑️Delete</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </table>
