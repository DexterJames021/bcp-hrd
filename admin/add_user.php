...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($check->num_rows > 0) {
        ...
        $sql = "INSERT INTO users (username, password, usertype, email, first_name, last_name, phone, address) 
                VALUES ('$username', '$password', '$usertype', '$email', '$first_name', '$last_name', '$phone', '$address')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check for existing username/email (optional but good practice)
    $check = $conn->query("SELECT * FROM users WHERE username='$username' OR email='$email'");
    if ($check->num_rows > 0) {
        echo "<script>alert('Username or Email already exists!');</script>";
    } else {
